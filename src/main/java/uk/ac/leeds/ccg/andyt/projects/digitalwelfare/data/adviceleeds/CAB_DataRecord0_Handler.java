 ...
 * @author geoagdt
 */
public class CAB_DataRecord0_Handler {

    public CAB_DataRecord0_Handler() {
    }
     ...
     * @param type 
     * if (type == 0) {
     * record = new CAB_DataRecord0(RecordID, line, this);
     * } else {
     * record = new CAB_DataRecord01(RecordID, line, this);
     * }                    }
     * @param filename
     * @param pw
     * @return TreeMap<String,Chapeltown_DataRecord> representing records
     */
    public TreeMap<String, CAB_DataRecord0> loadInputData(
            int type,
            String filename) {
...
//                }
                ...
                switch (tokenType) {
                    ...
                    case StreamTokenizer.TT_WORD:
                        line = st.sval;
                        try {
                            CAB_DataRecord0 record;
                            if (type == 0) {
                                record = new CAB_DataRecord0(RecordID, line, this);
                            } else {
                                record = new CAB_DataRecord01(RecordID, line, this);
                            }
                            String aClient_Ref = record.getClient_Ref();
                            if (result.containsKey(aClient_Ref)) {
//                                System.out.println("Additional record for client " + aClient_Ref);
                                additionalRecordsForClientsThatAreIgnored ++;
                            } else {
                                result.put(aClient_Ref, record);
                                recordsLoaded ++;
                            }
                        } catch (Exception e) {
